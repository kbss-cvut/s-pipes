...
public class SPipesExecutionOrder {
    private static Map<Node, List<Node>> edges = null;

    public SPipesExecutionOrder(Map<Node, List<Node>> edges) {
        SPipesExecutionOrder.edges = edges;
    }

    protected static List<Node> compute() {
    ...
    private static Comparator<QueueEntry> queueComparator() {
        return Comparator
                .comparingInt(QueueEntry::step).reversed()
                .thenComparing(e -> e.node().toString());
    }

    private static void processSuccessors(Node u, int step,
    ...
    private static Map<Node, Integer> computeIndegree() {
