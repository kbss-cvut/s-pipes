...
MIGRATION_JAR=$(find "$MODULE_DIR"/target -maxdepth 1 -name 's-pipes-migration-*.jar' ! -name '*-sources.jar' ! -name '*-javadoc.jar' ! -name '*-tests.jar' 2>/dev/null | head -1)
...
if [ -z "$MIGRATION_JAR" ]; then
    ...
    if [ -n "$NEWEST_SRC" ]; then
        echo "Sibling module sources (e.g. $NEWEST_SRC) are newer than JAR, rebuilding whole project..." >&2
        mvn -f "$MODULE_DIR"/../pom.xml package -DskipTests -q || exit 1
        MIGRATION_JAR=$(find "$MODULE_DIR"/target -maxdepth 1 -name 's-pipes-migration-*.jar' ! -name '*-sources.jar' ! -name '*-javadoc.jar' ! -name '*-tests.jar' 2>/dev/null | head -1)
    fi
elif [ -d "$SRC_DIR" ]; then
    ...
    if [ -n "$NEWEST_SRC" ]; then
        echo "Source files (e.g. $NEWEST_SRC) are newer than JAR, rebuilding s-pipes-migration..." >&2
        mvn -f "$MODULE_DIR"/pom.xml package -DskipTests -q || exit 1
        MIGRATION_JAR=$(find "$MODULE_DIR"/target -maxdepth 1 -name 's-pipes-migration-*.jar' ! -name '*-sources.jar' ! -name '*-javadoc.jar' ! -name '*-tests.jar' 2>/dev/null | head -1)
    fi
else
